...
The **course maintainer** owns the quarterly audit. Run it during the first week of
January, April, July, and October, and before a release after a long gap. A second
reviewer should approve changes to learner-facing safety guidance, executable
commands, authentication, billing, or production deployment.
...
2. **External facts** show the last review date, next due date, review question, and
   primary evidence links. Missing anchors and overdue reviews fail the audit, but a
   maintainer must consult the sources. The script never substitutes an HTTP status
   or an automated edit for that judgment.
...
3. For every due external fact, open each evidence link and capture the relevant
   product version, page title, and access date in the pull request.
...
5. Update that fact's `last_reviewed` value in `scripts/audit_course.py`. Change its
   anchor only when the learner-facing claim changed too.
6. Run `make audit`, `make build`, and inspect the diff. Have the second reviewer
   confirm high-impact changes before merge.

The pull request is the durable evidence record. It should identify the fact IDs,
primary sources, commands tested, environment, results, and any follow-up issue.
Do not paste credentials, private account data, or provider invoices into evidence.
