# Join the Cohort

The full course is free and open source — read every lesson, run every lab, ship the capstone, no paywall, no gatekeeping. That's the mission.

The **cohort** is for people who want to move faster, together.

## What you get

<div class="course-grid" markdown>

<div class="course-card" markdown>
### Live build sessions
Four weeks of live workshops working through the course tracks — foundations to agent teams — building in real time, questions answered on the spot.
</div>

<div class="course-card" markdown>
### Capstone reviews
Submit your capstone and get a real review: what's strong, what's weak, what a client or hiring manager would actually think.
</div>

<div class="course-card" markdown>
### The community
A private space of builders on the same path — accountability partners, code reviews, wins celebrated, blockers cleared. The network is half the value.
</div>

<div class="course-card" markdown>
### Templates and playbooks
Our CLAUDE.md library, Skill packs, MCP configurations, and launch checklists — the working files, not screenshots of them.
</div>

</div>

## Pricing

<div class="course-grid" markdown>

<div class="course-price" markdown>
**Self-paced**
<span class="price">Free</span>
Every lesson. Every lab. The capstone. Forever.

[Start now](../lessons/01_the_shift/README.md){ .md-button }
</div>

<div class="course-price" markdown>
**Cohort**
<span class="price">$149</span>
4 weeks live · capstone review · community · templates

[Reserve a seat](#enroll){ .md-button .md-button--primary }
</div>

<div class="course-price" markdown>
**Team**
<span class="price">$999</span>
Up to 10 seats · private sessions · your codebase, your context

[Talk to us](#enroll){ .md-button }
</div>

</div>

## Enroll { #enroll }

Cohorts run monthly and seats are capped so reviews stay real.

<!-- Wire your checkout link here (Stripe Payment Link, Gumroad, or Whop) -->
**[→ Reserve your seat for the next cohort](https://github.com/thetechhustle)**

Questions first? Open a [discussion on GitHub](https://github.com/thetechhustle/build_with_ai/discussions) and ask anything.

## FAQ

??? question "I'm not technical at all. Is this really for me?"
    Yes — you're exactly who the no-code lane was built for. Chapter 3 walks you into the terminal step by step, and every concept is introduced before it's used. If you can follow a recipe, you can follow this course.

??? question "I'm a senior engineer. Will this be too basic?"
    The engineer lane goes deep: context engineering for large codebases, Skills, MCP, hooks, subagents, worktree parallelism, and agentic harnesses. Skim the foundations in an evening, then Chapters 4–10 will earn their keep.

??? question "Why is the course free if the cohort costs money?"
    Knowledge shouldn't be gated. The lessons are free forever. The cohort sells time, feedback, and community — things that don't scale for free.

??? question "What does it cost to run the labs?"
    You'll need a Claude plan or a small API budget. Most learners complete the labs comfortably on a Pro subscription. Chapter 11 teaches cost management explicitly.

??? question "Do I need a Mac?"
    No. Mac, Windows (with WSL), and Linux all work. Setup covers all three.

??? question "How current is the material?"
    The course is maintained in the open on GitHub. When the tools move, the lessons move — check the commit history for receipts.
