...
Why this shape is gold: **agents honor reasons far better than rules.** A rule ("no frameworks") invites clever exceptions; a reason ("client edits it herself") lets the agent generalize correctly to situations the rule never anticipated. And `REJECTED:` prevents the classic groundhog loop — the agent (or a future you) proposing last month's rejected idea fresh next quarter.
...
1. Create `docs/` with the four files in your quick-win project. Seed `decisions.md` with two decisions you've *already* made (you have — think: no frameworks? single page? color scheme?). Use the four-line format, `REJECTED:` included.
