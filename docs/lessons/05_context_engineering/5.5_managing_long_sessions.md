...
=== "⚙️ Engineer lane"
    ...
    Extra lever: **keep exploration out of the build session.** Big investigations (codebase spelunking, log archaeology) generate enormous low-value context. Do them in a throwaway session — or better, a subagent (Chapter 10), which explores in *its own* window and returns only conclusions. Your build session's context stays curated: plan, relevant files, work. Context budgeting like this is 80% of what "good at long agentic tasks" means in practice.
