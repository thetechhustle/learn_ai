...
| Daily building | Workhorse | A sensible starting hypothesis; validate it on your tasks |
| Architecture, hard debugging, split-planning (10.3), anything where a wrong call is expensive | Max | Higher capability may reduce rework, but still needs verification |

Claude Code lets you set and switch models (`/model`, and per-subagent in role definitions). Treat the table as an experiment plan, not law: compare quality, retries, latency, and usage on representative tasks before setting team defaults.
