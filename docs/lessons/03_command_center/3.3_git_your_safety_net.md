...
=== "Windows PowerShell"
    ...
    ```powershell
    Set-Content -Path .\README.md -Value "TERRIBLE IDEA"
...
A remote repository keeps another copy of pushed commits. It can be private or
public, and it still depends on your account, repository availability, and
which commits you actually pushed. Public repositories may support a portfolio;
...
Refresh the GitHub page: the commits you pushed are now in that remote
repository. Confirm its visibility setting. A private repository is the
default choice for practice that is not intentionally public.
...
3. Review with `git diff`, then run `git add notes/brief.txt` and
   `git commit -m "Add brief line"` as two separate commands.
...
6. If you configured an authorized remote, push and inspect its commit history.
   Record whether the repository is private or public.
