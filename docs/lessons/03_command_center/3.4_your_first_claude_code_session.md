...
You're now at the Claude Code prompt — same terminal, new conversation partner, and this folder is its whole world. Say hello properly. Type:
...
Then verify with your own eyes — exit Claude Code (`/exit`) or open a second terminal:
...
`git diff` shows every changed line. You may not read HTML fluently yet — skim anyway. Patterns start forming from day one, and *looking* is the habit that matters.
...
```console
$ git add . && git commit -m "First agent-built page: personal site v1"
$ git push
```

Snapshot taken. The experiment is banked. And that push just put your first agent-built work on your public GitHub.
...
- Genuinely broken → you have a snapshot. `git restore .` and run it back with a sharper brief. The rollback costs nothing — that was the whole point of Lesson 3.3.
