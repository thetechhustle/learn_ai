...
4. Run scripts/word_count.py on the draft — target 550–650 words.
...
**Scripts** are the sleeper feature: a Skill can carry executable tools. The `word_count.py` above is trivial, but the principle isn't — deterministic steps (counting, validating, converting, fetching) run as *code, not judgment*. The agent doesn't estimate the word count; it runs the counter. Skills that mix instructions-for-judgment with scripts-for-mechanics get the best of both kinds of reliability. (Remember 2.4 — models stumble at precision; scripts don't.)
